...
# NixOS: the venv's compiled extensions (pandas, soundfile) dlopen
# libstdc++.so.6, which nix-ld's loader does not cover for dlopen from an
# already-running interpreter. NIX_LD_LIBRARY_PATH holds it on this host.
...
# The artifacts live one level above the ninfer checkout; ninfer/out/ holds
# only a symlink into the deleted ephemeral copy (see ../LOG.md).
artifact="${repo_dir}/../out/qwen3_8_27b_nvfp4_uncensored.ninfer"
config="${repo_dir}/eval/configs/qwen3_8_27b_nvfp4_uncensored_reasoning.yaml"
...
run_step() {
    ...
    server_log="${log_dir}/qwen3_8_27b_nvfp4_uncensored-${step}-c2-${run_stamp}.server.log"
    request_log="${log_dir}/qwen3_8_27b_nvfp4_uncensored-${step}-c2-${run_stamp}.requests.jsonl"
