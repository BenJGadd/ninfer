...
Same probes as the Qwen3.6-27B verifier; the three geometry-dependent literals
(GDN value/z split rows, the convolution alias shape, the draft-head width)
are derived from :mod:`tools.convert.qwen3_5_9b.geometry`.
"""
...
from . import convert, draft_head, inventory, recipe
from .geometry import GEOMETRY


PROJECT_ROOT = Path(__file__).resolve().parents[3]

_FIRST_FULL = GEOMETRY.full_attention_layers[0]
_FIRST_GDN = GEOMETRY.gdn_layers[0]

DIRECT_PROBE_OBJECTS = (
    f"text/layers/{_FIRST_GDN}/input_norm",
    f"text/layers/{_FIRST_GDN}/gdn/a_log",
    f"text/layers/{_FIRST_GDN}/gdn/convolution",
)
QUANT_PROBE_OBJECTS = (
...
class VerificationError(ValueError):
    """The artifact does not match the registered 9B target contract."""
    ...
    for alias in inventory.ALIAS_SPECS:
        ...
        for layer in layers:
            ...
            if alias.axis_order is not None:
                ...
                if target_shape != _CONVOLUTION_ALIAS_SHAPE:
...
def validate_draft_token_ids(token_ids: torch.Tensor) -> None:
    if token_ids.dtype != torch.int32 or tuple(token_ids.shape) != (draft_head.DRAFT_HEAD_N,):
        _contract_error(f"draft token IDs must be I32[{draft_head.DRAFT_HEAD_N}]")
...
def _verify_resources_and_frontend(
    ...
    expected = {
        ...
        for resource in convert.load_resources(model_dir, generation_config)
    }
    ...
    for spec in inventory.RESOURCE_SPECS:
        ...
        if payload != expected[spec.name]:
            _contract_error(f"frontend resource differs from source: {spec.name}")
        payloads[spec.name.removeprefix("frontend/")] = payload
...
def verify_payloads(
    ...
    model_dir: str | Path,
    device: str | torch.device = "cpu",
    generation_config: str | Path = convert.GENERATION_CONFIG_PATH,
) -> PayloadSummary:
...
def verify_artifact(
    ...
    payload = verify_payloads(artifact, model_dir, device, generation_config)
    return VerificationSummary(structure=structure, payload=payload)


def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Verify a Qwen3.5-9B NInfer artifact against its source checkpoint"
    )
    ...
    parser.add_argument(
        "--generation-config",
        type=Path,
        default=convert.GENERATION_CONFIG_PATH,
    )
    return parser
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    with Artifact.open(arguments.artifact) as artifact:
        summary = verify_artifact(
            artifact, arguments.model, arguments.device, arguments.generation_config
        )
