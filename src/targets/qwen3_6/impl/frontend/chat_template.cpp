...
// Qwen3.5-9B ships the ThinkingToggle template minus the `preserve_thinking` branch and
// with scalar tool arguments rendered by `string` rather than `tojson`; the native renderer
// implements the Qwen3.6 superset. (Upstream master executes jinja directly and does not
...
CompiledChatTemplate CompiledChatTemplate::resolve(std::string_view source) {
    ...
    if (digest == kThinkingToggleTemplateDigest ||
        digest == kQwen35ThinkingToggleTemplateDigest) {
