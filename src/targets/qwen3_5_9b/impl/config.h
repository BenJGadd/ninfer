...
// Qwen3.5-9B compile-time geometry. The eight primitive dimensions below are the
// same ones registered in tools/convert/qwen3_5_9b/geometry.py; every other
// extent is derived here exactly as that module derives it. Keep the two in step
// (tests/convert/qwen3_5_9b/test_config_h.py compares them).
...
struct TextConfig {
    ...
    static constexpr int rotary_dim  = head_dim / 4; // partial_rotary_factor 0.25
    ...
    // Artifact parents that fuse two logical projections along their row axis.
    static constexpr int attention_query_key_rows = query_size + kv_size;
    static constexpr int gdn_query_key_rows       = 2 * key_dim;
    static constexpr int gdn_value_z_rows         = 2 * value_dim;
    static constexpr int mlp_gate_up_rows         = 2 * intermediate;
...
static_assert(TextConfig::rotary_dim == 64);

struct VisionConfig : qwen3_6::VisionBackboneConfig {
    static constexpr int output_hidden = TextConfig::hidden;
};

// No masked-draft (DFlash/DFlash2) companion exists for Qwen3.5-9B. The family runtime
// keys every DFlash path on `supported`; `backend = None` makes `--spec dflash*` an
// explicit "not supported by this target" error at startup. The remaining members
// exist only so discarded template branches still name-resolve.
struct DFlashConfig {
...
inline constexpr float kAttentionScale                   = 0.0625F;             // 1/sqrt(head_dim)
inline constexpr float kGdnScale                         = 0.08838834764831845F; // 1/sqrt(gdn_key_head_dim)
...
static_assert(TextConfig::head_dim == 256 && TextConfig::gdn_key_head_dim == 128,
              "kAttentionScale/kGdnScale are written for head widths 256 and 128");
