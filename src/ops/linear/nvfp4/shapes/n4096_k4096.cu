...
namespace {
using Geometry = Nvfp4Geometry<4096, 4096>;
using Gemv =
...
void reject_a4(const Tensor&, const Weight&, Tensor&, Nvfp4W4a4Workspace, cudaStream_t) {
    throw std::logic_error("nvfp4 4096x4096: A4 route is not registered (weight-only shape)");
}

bool uses_a4(std::int32_t, std::int32_t) { return false; }
} // namespace

const Nvfp4LinearShape kNvfp4N4096K4096{4096, 4096, launch_nvfp4_a16_chunks<32, select_a16>, reject_a4,
                                        uses_a4};
} // namespace ninfer::ops::detail
